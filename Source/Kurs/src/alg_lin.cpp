...
void AlgLin::resolve_r()
{
    ...
    for(i=0; i<nx; i++) {
        for(j=0; j<ny; j++) {
            ...
            r2[i][j]=(u[i][j]-u_temp1[i][j])/t+g*diffX(pov,i,j);
            r3[i][j]=(v[i][j]-v_temp1[i][j])/t+g*diffY(pov,i,j);
        }
    }

}

void AlgLin::resolve_r(int mode)
{
    register size_t i,j;
    //создаем дополнительные массивы
    #pragma omp parallel for collapse(2)
...
double* AlgLin::find_all(double **a,double **b,double **c)
{
    double temp=0;
    double arr=0;
    double arar=0;
    ...
    for(i=0; i<nx; i++) {
        for(j=0; j<ny; j++) {
            ...
            temp=(b[i][j])/t+g*diffX(a,i,j);
            arr=temp*r2[i][j];
            arar=temp*temp;
            p2+=arr;
            q2+=arar;

            temp=(c[i][j])/t+g*diffY(a,i,j);
            arr=temp*r3[i][j];
            arar=temp*temp;
            p3+=arr;
            q3+=arar;
...
double AlgLin::find(double **a,double **b,double **c)
{
    double temp=0;
    double arr=0;
    double arar=0;
...
double AlgLin::find(int mode,double **a,double **b,double **c)
{
    double temp=0;
    double arr=0;
    double arar=0;
